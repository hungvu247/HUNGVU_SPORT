...
public class Statistic extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<OrderTable> list = od.getAllOrder();
        AdminDAO d = new AdminDAO();
        double totalAmount = 0;

        for (OrderTable orderTable : list) {
            totalAmount += orderTable.getTotal();
        }
        for (OrderTable orderTable : list) {

        }
        int numberOfOrder = od.countOrder();
        int numberOfProduct = od.countTotalProduct();
        request.setAttribute("numberOfOrder", numberOfOrder);
        request.setAttribute("totalReview", rd.countView());
