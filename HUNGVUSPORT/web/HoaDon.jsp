...
<html>
    ...
    <body>
                        ...
                        <div class="card-header py-3 row">
                            <div class="col-sm-6">
                                <h5 class="mb-0 text-left" id="">
                                    <strong> <a href="invoicecontroller" class="list-group-item list-group-item-action py-2 ripple"><i class="fas fa-file-invoice-dollar fa-fw me-3"></i><span>Hóa Đơn</span></a></strong>
                                </h5>
                            </div>
                            <div class="col-sm-6">
                                ...
                                <table class="table table-hover text-nowrap">
                                    ...
                                    <tbody id="content">
                                        <c:forEach items="${listAllInvoice}" var="invoice">
                                            <tr>
                                                <td></td>
                                                <td>${invoice.getOrder_id()}</td>
                                                <c:forEach items="${listAllUser}" var="user">
                                                    <c:if test="${invoice.getCus_id() == user.getId()}">
                                                        <td>${user.getUsername()}</td>
                                                    </c:if>
                                                </c:forEach>
                                                <td>${String.format("%.2f", invoice.getTotal())}</td>
                                                <td>${invoice.getOrder_date()}</td>
                                                <td>  <a href="#" class="check"style="color:green;margin-right:10px">&#10003;</a>
                                                    <a href="delete" class="cross" onclick="showInput()"style="color:red">✕</a>
                                                    <div id="inputContainer" style="display: none;">
                                                        <input type="text" id="textInput" placeholder="Nhập lý do hủy...">
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    console.log("Phản hồi từ servlet:", xhr.responseText);
                    // Đóng thẻ input
        ...
        xhr.send('inputData=' + encodeURIComponent(inputData));
    }
        </script>
        ...
        <script>
            function searchByDate(param) {
                ...
                $.ajax({
                    url: "/hungvusport/searchAjaxHoaDon",
